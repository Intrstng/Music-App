...
> = async (args, api, extraOptions) => {
    ...
    // Если запрос завершился ошибкой 401 Unauthorized т.е. токен "истёк"
        ...
        if (!mutex.isLocked()) {
            ...
            try {
                ...
                if (refreshResult.data && isTokens(refreshResult.data)) {
                    ...
                    // Если обновление токена не удалось — выполняем выход из системы - чтобы исключить перебор токенов мошенником
                    // @ts-expect-error
